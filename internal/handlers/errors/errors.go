...
func InvalidRequest() *errorBuilder {
	return &errorBuilder{httpStatusCode: http.StatusBadRequest, errorCode: string(ErrInvalidRequest), message: "Invalid Request"}
}

func Unauthorized() *errorBuilder {
	return &errorBuilder{httpStatusCode: http.StatusUnauthorized, errorCode: string(ErrUnauthorized), message: "Unauthorized"}
}

func NotFound() *errorBuilder {
	return &errorBuilder{httpStatusCode: http.StatusNotFound, errorCode: string(ErrNotFound), message: "Not Found"}
}

func NotAllowed() *errorBuilder {
	return &errorBuilder{httpStatusCode: http.StatusMethodNotAllowed, errorCode: string(ErrNotAllowed), message: "Not Allowed"}
}

func InternalServerError() *errorBuilder {
	return &errorBuilder{httpStatusCode: http.StatusInternalServerError, errorCode: string(ErrInternalServerError), message: "Internal Server Error"}
}

func ServiceNotAvailable() *errorBuilder {
	return &errorBuilder{httpStatusCode: http.StatusServiceUnavailable, errorCode: string(ErrServiceUnavailable), message: "Service Not Available"}
}
